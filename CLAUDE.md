...
Google Apps Script(`Code.gs`) + Google Sheets는 백업 용도로만 잔존 (모든 변경사항 실시간 자동 백업 + 필요 시 스마트 동기화로 복구).
...
- **시트 z-index 동적 스태킹** (trip_hj와 동일 패턴, 세 앱 통일): `initBackGuard()`의 MutationObserver가 오버레이 `.show` 변화를 감지해 `manageZ()`로 **열린 순서대로 z-index(`350+seq`)를 부여** — 늦게 연 오버레이가 항상 위(DOM 순서·CSS 정적 z-index 무관). 다른 시트 위에 겹쳐 떠도 가려지지 않게 하는 근본책. `Z_OVERLAYS` 목록(모달 오버레이만, `monthly-search-wrap` 같은 인라인 요소 제외)으로 관리하며, 호출부(`classList.add('show')`)는 그대로 두고 중앙에서 처리. 신규 모달 오버레이 추가 시 `LAYERS`와 `Z_OVERLAYS`에 함께 등록할 것.
...
| 시트 백업이 지연되거나 실패함 | 일시적 네트워크 오류 | 데이터 관리 → **동기화 진단하기** → 동기화 실행 (smartSync가 수동으로 강제 동기화 수행) |
